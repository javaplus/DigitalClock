...
int clockMinuteColour = 1677721;
// int clockMinuteColour = 51200; //1677
int clockHourColour = 1677721; //7712; //7712
...
void setup() {
  ...
  stripClock.setBrightness(75); // Set inital BRIGHTNESS (max = 255)
  ...
  //smoothing
    // initialize all the readings to 1000  
  for (int thisReading = 0; thisReading < numReadings; thisReading++) {
    readings[thisReading] = 1000;
  }
...
void loop() {
  ...
  clockFaceBrightness = map(lightSensorValue,300, 950, 240 , 0); 
  //set the brightness based on ambiant light levels
  if(lightSensorValue <= 300){
    // it's really bright
    clockFaceBrightness = 240;
  }
  if(lightSensorValue >= 950){
    // it's really dark
  ...
  Serial.print("Updated brightness value = ");
  Serial.println(clockFaceBrightness);
  stripClock.show();
  ...
  delay(200);   //this 5 second delay to slow things down during testing
...
void displayTheTime(){
  ...
  if(firstHourDigit == 0){
    firstHourDigit = 2; // it's midnight
  }else{
  ...
  if (secondHourDigit > 12){
    ...
    if (secondHourDigit > 9){
      stripClock.fill(clockHourColour,84, 8); 
    }
    if(secondHourDigit==0){ // It's midnight
      stripClock.fill(clockHourColour,84, 8); 
    }
